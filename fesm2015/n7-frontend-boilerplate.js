...
class AwMapDS extends DataSource {
    constructor() {
        ...
        this.transform = (data) => ({
            ...
            _setInstance: (map) => {
                this.map = map;
                const bounds = new LatLngBounds(data
...
class FacetLinkDS extends DataSource {
    ...
    setValue(value, update = false) {
        ...
        if (update) {
            ...
            const updatedLinks = links.map((link) => (Object.assign(Object.assign({}, link), { classes: this.value && (this.value === link.payload) ? ACTIVE_CLASS$1 : '' })));
