    ...
    transform(data) {
        ...
        data.form.config.groups = groups.map((group) => ({
            ...
            options: {
                ...
                payload: group.id,
                iconRight: group.options.isOpen ? ICON_OPEN$1 : ICON_CLOSE$1,
    ...
    setValue(value, update = false) {
        ...
        if (update) {
            ...
            const updatedLinks = links.map((link) => ({
                ...link,
                classes: this.value && (this.value === link.payload) ? ACTIVE_CLASS$2 : ''
            }));
            this.update({
                ...this.input,
                links: updatedLinks
...
class FacetLinkMultipleDS extends DataSource {
    ...
    setValue(value, update = false) {
        ...
        if (update) {
            ...
            const updatedLinks = links.map((link) => ({
                ...link,
                classes: this.value.includes(link.payload) ? ACTIVE_CLASS$1 : ''
            }));
            this.update({
                ...this.input,
                links: updatedLinks
